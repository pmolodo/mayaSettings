if not globals().get('_USER_SETUP_EXECUTED', False):
    ...
    if IMPORT_PYMEL:
        ...
        if pymelImported:
            ...
            try:
                from pymel_test import nose_test as pymel_test
            except Exception:
    ...
    mayaVersion = None
    try:
        mayaVersion= PMP.mayaUtils.getMayaVersion()
    except Exception:
        print "Error retrieving mayaVersion:"
        import traceback
        traceback.print_exc()

    if mayaVersion is not None:
        pathsToAdd.extend([r"C:\Program Files\Autodesk\MayaBonusTools%d\python"  % mayaVersion,
                 r"C:\MayaBonusTools%d\python" % mayaVersion])
    if 'OMTOOLBOX' in os.environ:
        pathsToAdd.append(os.path.join(os.environ["OMTOOLBOX"], "pyScripts"))
    if 'PAUL_SCRIPTS' in os.environ:
        pathsToAdd.append(os.path.join(os.environ["PAUL_SCRIPTS"], "pyScripts"))
    if 'PAUL_C_DRIVE' in os.environ:
        pathsToAdd.append(os.path.join(os.environ["PAUL_C_DRIVE"],
